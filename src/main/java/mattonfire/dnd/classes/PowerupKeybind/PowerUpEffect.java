...
public class PowerUpEffect {
    public static void play(PlayerEntity player, int classID) {
        ...
        switch (classID) {
            case 12:
                player.getEntityWorld().createExplosion(null, player.getX(), player.getY(), player.getZ(), 10.F, true, DestructionType.BREAK);
                break;
            case 11:
                // Vec3d aim = player.getVelocity();
                // FireballEntity fireball = new FireballEntity(player.world, player, 1, 1, 1);
                // fireball.refreshPositionAndAngles(player.getX() + aim.x * 1.50, player.getY() + aim.y * 1.50, player.getZ() + aim.z * 1.50, 0.F, 0.F);

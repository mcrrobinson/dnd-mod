...
public class DnDClasses implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        Registry.register(Registry.STATUS_EFFECT, new Identifier("dndclasses", "super_strength"),
                new SuperStrengthStatusEffect());
        Registry.register(Registry.ITEM, new Identifier("dndclasses", "staff_of_fire"),
                new ExtendedSwordItem(new ToolMaterialFabric(), new Item.Settings().group(ItemGroup.TOOLS)));
        Registry.register(Registry.ITEM, new Identifier("dndclasses", "staff_of_lightning"),
                new ExtendedSwordItem(new ToolMaterialFabric(), new Item.Settings().group(ItemGroup.TOOLS)));
        Registry.register(Registry.ITEM, new Identifier("dndclasses", "staff_of_ice"),
                new ExtendedSwordItem(new ToolMaterialFabric(), new Item.Settings().group(ItemGroup.TOOLS)));

        Registry.register(Registry.ITEM, new Identifier("dndclasses", "monk_staff"),
        ...
        ServerPlayNetworking.registerGlobalReceiver(C2S_DOUBLEJUMP_EFFECTS_REQUEST_PACKET_ID,
                (server, player, handler, buf, responseSender) -> {
                    ...
                    server.execute(() -> {
                        Stream<PlayerEntity> watchingPlayers = PlayerStream.watching(entity.getEntityWorld(),
                                entity.getBlockPos());
                        watchingPlayers.forEach(p -> {
                            if (p != player) {
                                ServerPlayNetworking.send((ServerPlayerEntity) p,
                                        DnDClasses.S2C_DOUBLEJUMP_EFFECTS_PACKET_ID, passedData);
                            }
                        });
                    });
                });
